...
enemy_x_pos = (screen_width - enemy_width) / 2 # 화면 가로의 중간 지점에 위치 (가로 위치)
enemy_y_pos = (screen_height - enemy_height) / 2 # 화면 세로의 중간 지점에 위치 (세로 위치)
...
while running:
    ...
    # 충돌 체크
    if character_rect.colliderect(enemy_rect): # 충돌이 있었는지 알려주는 함수
        print("충돌했어요")
        running = False
